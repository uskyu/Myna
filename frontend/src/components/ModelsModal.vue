...
.model-toolbar { display: flex; gap: 8px; margin-bottom: 14px; }
...
.batch-count { flex: 1; min-width: 100px; font-size: 12px; color: var(--text-dim); }
.danger-link { color: var(--danger); }
.link-mini:disabled { opacity: 0.5; cursor: not-allowed; text-decoration: none; }
.model-row.selected { border-color: var(--accent); background: var(--accent-soft); }
.model-select-box { width: 16px; height: 16px; margin-top: 4px; accent-color: var(--accent); flex-shrink: 0; }

@media (max-width: 520px) {
  .model-toolbar { flex-direction: column; }
  .batch-toggle { width: 100%; }
  .batch-bar { align-items: stretch; }
  .batch-bar .link-mini { padding: 6px 8px; border: 1px solid var(--border); }
}
